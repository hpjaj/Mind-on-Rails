...
RSpec.describe Note, :type => :model do
  ...
  describe "validations" do
    it "ensures title is at least 5 characters" do
      expect(Note.new).to validate_length_of(:title).is_at_least(5).with_message(/ - Minimum length of 5 characters/)
    end

    it "ensures body is at least 5 characters" do
      expect(Note.new).to validate_length_of(:body).is_at_least(5).with_message(/ - Minimum length of 5 characters/)
    end
